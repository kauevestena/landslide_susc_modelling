...
def infer_with_tta(model: nn.Module, tensor: torch.Tensor, tta: bool) -> np.ndarray:
    ...
    transformations = [
        ...
        (lambda x: torch.rot90(x, k=1, dims=[-2, -1]), lambda y: np.rot90(y, k=3, axes=(1, 2))),
        (lambda x: torch.rot90(x, k=3, dims=[-2, -1]), lambda y: np.rot90(y, k=1, axes=(1, 2))),
    ]
    ...
    with rasterio.open(feature_stack_path) as src:
        ...
        for y in range(0, height, stride):
            for x in range(0, width, stride):
                ...
                tensor = torch.from_numpy(np.ascontiguousarray(tile)).unsqueeze(0).to(device)
...
def compute_uncertainty(mc_stack: Optional[np.ndarray], probabilities: np.ndarray, positive_class: int, mask: np.ndarray) -> np.ndarray:
    """Estimate pixel-level uncertainty from MC samples or entropy fallback."""
    if mc_stack is not None and mc_stack.size > 0:
        uncertainty = np.std(mc_stack[:, positive_class, :, :], axis=0).astype(np.float32)
    else:
...
def write_geotiff(path: str, array: np.ndarray, reference_path: str, dtype: str, nodata: Optional[float] = None) -> None:
...
def run_inference(config: Dict, artifacts: Dict[str, 'AreaArtifacts'], training_artifacts: Dict[str, Optional[str]], force_recreate: bool = False) -> None:
    ...
    uncertainty_map = compute_uncertainty(mc_stack, probabilities, positive_class, valid_pixels)
    ...
    write_geotiff(susceptibility_path, susceptibility, area.feature_stack_path, dtype='float32', nodata=0.0)
    write_geotiff(uncertainty_path, uncertainty_map, area.feature_stack_path, dtype='float32', nodata=0.0)
    write_geotiff(class_map_path, class_map, area.feature_stack_path, dtype='uint8', nodata=255)
    write_geotiff(valid_mask_path, valid_pixels.astype(np.uint8), area.feature_stack_path, dtype='uint8', nodata=0)
