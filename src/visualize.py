...
def plot_training_history(
    ...
    for idx, metric_name in enumerate(metric_names[:3]):  # Plot first 3 additional metrics
        if metric_name in val_metrics_keys:
            row = (idx + 1) // 2
            col = (idx + 1) % 2
            values = [entry["val_metrics"].get(metric_name, float('nan')) for entry in history]
            axes[row, col].plot(epochs, values, 'g-d', linewidth=2)
            axes[row, col].set_xlabel('Epoch', fontsize=12)
            axes[row, col].set_ylabel(metric_name.upper().replace('_', ' '), fontsize=12)
            axes[row, col].set_title(f'Validation {metric_name.upper()}', fontsize=14, fontweight='bold')
