...
class LandslideDataset(Dataset):
    ...
    def __init__(self, tiles_dir: str, labels_dir: str, split: str, augment_config: Optional[Dict] = None):
        import glob
    ...
    def _apply_augmentations(self, tile: np.ndarray, label: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
class DiceCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, num_classes: int, weight: Optional[torch.Tensor] = None, ignore_index: int = IGNORE_INDEX):
        super().__init__()
        self.ce = nn.CrossEntropyLoss(weight=weight, ignore_index=ignore_index)
        self.dice = smp.losses.DiceLoss(mode='multiclass', from_logits=True, ignore_index=ignore_index)

    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        return 0.5 * self.ce(inputs, targets) + 0.5 * self.dice(inputs, targets)


def compute_class_weights(summary_path: str, num_classes: int) -> Optional[torch.Tensor]:
...
def compute_metrics_from_confusion(confusion: np.ndarray, correct: int, total: int, prob_list: Optional[List[np.ndarray]] = None, label_list: Optional[List[np.ndarray]] = None) -> Dict[str, float]:
    ...
    metrics['macro_iou'] = float(np.mean(per_class_iou)) if per_class_iou else float('nan')
...
def evaluate(model: nn.Module, loader: Optional[DataLoader], loss_fn: nn.Module, device: torch.device, num_classes: int, positive_class: int, collect_probs: bool = False) -> Tuple[float, Dict[str, float], Tuple[Optional[np.ndarray], Optional[np.ndarray]]]:
    """Run evaluation on a dataloader and gather metrics, optionally storing probabilities."""
    if loader is None or len(loader) == 0:
        metrics = {'overall_accuracy': float('nan'), 'macro_iou': float('nan'), 'macro_f1': float('nan'), 'auroc': float('nan'), 'auprc': float('nan')}
    ...
    with torch.no_grad():
        for tiles, labels in loader:
            ...
            if collect_probs:
                ...
                label_list.append((labels_valid == positive_class).cpu().numpy().astype(np.uint8))

    avg_loss = total_loss / max(len(loader), 1)
    metrics = compute_metrics_from_confusion(confusion, correct, total, prob_list if collect_probs else None, label_list if collect_probs else None)
...
def train_model(config: Dict, train_artifacts, force_recreate: bool = False) -> Dict[str, Optional[str]]:
    ...
    if dropout_prob and dropout_prob > 0:
        model.segmentation_head = nn.Sequential(nn.Dropout2d(p=dropout_prob), model.segmentation_head)

    model = model.to(device)

    class_weights = compute_class_weights(os.path.join(splits_dir, 'dataset_summary.json'), num_classes)
    if class_weights is not None:
        class_weights = class_weights.to(device)

    loss_fn = DiceCrossEntropyLoss(num_classes, weight=class_weights, ignore_index=IGNORE_INDEX)
    ...
    for epoch in range(training_cfg['epochs']):
        ...
        for tiles, labels in train_loader:
            ...
            if use_amp:
                ...
                if training_cfg.get('gradient_clip_norm'):
                    ...
                    nn.utils.clip_grad_norm_(model.parameters(), training_cfg['gradient_clip_norm'])
        ...
        if score > best_score:
            ...
            torch.save({'state_dict': model.state_dict(), 'in_channels': in_channels, 'num_classes': num_classes}, best_model_path)
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
    ...
    if best_calibration and best_calibration[0] is not None and best_calibration[0].size > 0:
