...
def load_input_paths(config: Dict) -> Dict[str, AreaInputs]:
    ...
    for split_name, split_cfg in config['inputs'].items():
        ...
        debug_print(f"Registered inputs for split '{split_name}': DTM={dtm_path}, Ortho={ortho_path}, GroundTruth={ground_path}")
...
def save_geotiff(path: str, array: np.ndarray, reference_profile: Dict, dtype: Optional[str] = None, nodata: Optional[float] = None) -> None:
...
def compute_dem_features(elevation: np.ndarray, transform: Affine, valid_mask: np.ndarray, preprocessing_cfg: Dict) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    ...
    flow = d8_flow_accumulation(elevation, valid_mask, cellsize=(cellsize_x + cellsize_y) * 0.5)
    ...
    drainage_threshold = preprocessing_cfg['hydrology'].get('drainage_threshold', 5000.0)
...
def normalize_radiometry(ortho: np.ndarray, valid_mask: np.ndarray, epsilon: float) -> np.ndarray:
    ...
    for band_idx in tqdm(range(normalized.shape[0]), desc='normalize_radiometry', leave=False):
...
def derive_land_cover(ortho: np.ndarray, valid_mask: np.ndarray, clusters: int, seed: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if valid_flat.size == 0:
        debug_print('derive_land_cover: no valid pixels, returning empty land cover stack')
    ...
    if sample.shape[0] > max_samples:
        debug_print(f"derive_land_cover: subsampling from {sample.shape[0]} to {max_samples} pixels")
...
def build_feature_stack(dem_feature_map: Dict[str, np.ndarray], ortho_channels: np.ndarray, land_cover: np.ndarray, preprocessing_cfg: Dict, valid_mask: np.ndarray) -> Tuple[np.ndarray, List[str]]:
    ...
    for band_idx in tqdm(range(ortho_channels.shape[0]), desc='build_feature_stack: ortho bands', leave=False):
        debug_print(f"build_feature_stack: including normalized ortho band {band_idx + 1}")
        feature_arrays.append(ortho_channels[band_idx])
        channel_names.append(f'ortho_norm_band_{band_idx + 1}')

    for cls_idx in tqdm(range(land_cover.shape[0]), desc='build_feature_stack: land cover', leave=False):
...
def apply_normalization(stack: np.ndarray, valid_mask: np.ndarray, normalization_cfg: Dict, existing_stats: Optional[Dict]) -> Tuple[np.ndarray, Dict]:
    """Normalize the feature tensor and optionally reuse stored statistics."""
    strategy = normalization_cfg.get('strategy', 'zscore')
    debug_print(f"apply_normalization: strategy={strategy}, reuse_stats={existing_stats is not None}")
    ...
    if strategy == 'zscore':
        if stats is None:
            ...
            for channel_idx in tqdm(range(normalized.shape[0]), desc='apply_normalization', leave=False):
            ...
            for channel_idx in tqdm(range(normalized.shape[0]), desc='apply_normalization: reuse', leave=False):
...
def reproject_raster(src_path: str, reference_profile: Dict, resampling: Resampling, dst_dtype: Optional[np.dtype] = None) -> Tuple[np.ndarray, Optional[float]]:
...
def process_area(area_inputs: AreaInputs, area_dir: str, preprocessing_cfg: Dict, metadata_dir: str, stats: Optional[Dict], seed: int, force_recreate: bool = False) -> Tuple[AreaArtifacts, Optional[Dict]]:
    ...
    dtm_filled = simple_sink_fill(dtm_filled, preprocessing_cfg['dtm_hygiene'].get('sink_fill_kernel', 0))
    ...
    if gaussian_sigma and gaussian_sigma > 0.0:
        ...
        debug_print(f"process_area: applied Gaussian smoothing with sigma={gaussian_sigma}")

    dem_features, dem_meta = compute_dem_features(dtm_filled, profile['transform'], valid_mask, preprocessing_cfg)
    ...
    ground_truth_path = None
    if area_inputs.ground_truth:
        gt_resampling = resampling_map.get(preprocessing_cfg['resampling'].get('ground_truth', 'nearest'), Resampling.nearest)
        gt_array, _ = reproject_raster(area_inputs.ground_truth, profile, gt_resampling, dst_dtype=np.float32)
        gt = gt_array[0]
        gt = np.round(gt).astype(np.int16)
        gt[~valid_mask] = 255
        ignore_mask = compute_ignore_mask(gt, preprocessing_cfg.get('boundary_ignore_pixels', 0))
        gt[ignore_mask] = 255
        ground_truth_path = os.path.join(area_dir, 'ground_truth_aligned.tif')
        save_geotiff(ground_truth_path, gt.astype(np.uint8), profile, dtype='uint8', nodata=255)
...
def prepare_dataset(config: Dict, train_artifacts: AreaArtifacts, force_recreate: bool = False) -> None:
    ...
    labels = np.where(ignore_mask, 255, np.clip(labels, 1, config['model']['out_classes']) - 1)
    ...
    for split in ['train', 'val', 'test']:
        ...
        for path in glob.glob(os.path.join(structure_cfg['tiles_dir'], split, '*.npy')):
            os.remove(path)
        for path in glob.glob(os.path.join(structure_cfg['labels_dir'], split, '*.npy')):
            os.remove(path)
    ...
    if len(blocks) < 2 or test_size <= 0:
        ...
        train_blocks, test_blocks = train_test_split(blocks, test_size=effective_test_size, random_state=random_state)
    ...
    if val_size > 0 and len(train_blocks) > 1:
        ...
        if adjusted_val >= 1.0:
            adjusted_val = 0.5
        train_blocks, val_blocks = train_test_split(train_blocks, test_size=adjusted_val, random_state=random_state)
    else:
        val_blocks = []
    split_blocks = {
        'train': train_blocks,
    ...
    positive_class = dataset_cfg.get('positive_class', config['model']['out_classes'] - 1)
            ...
            for y in range(y0, max(y1 - tile_size + 1, y0), stride):
                for x in range(x0, max(x1 - tile_size + 1, x0), stride):
                    ...
                    if not np.any(valid_pixels):
                        continue
                    positive_fraction = np.mean(tile_labels[valid_pixels] == positive_class)
    ...
    class_pixel_counts: Dict[str, int] = {str(idx): 0 for idx in range(config['model']['out_classes'])}
    ...
    for split_name, groups in split_positions.items():
        ...
        if not pos_positions:
            ...
            neg_target = int(round(len(pos_positions) * (1.0 - desired_pos_fraction) / max(desired_pos_fraction, 1e-6)))
            neg_target = min(len(neg_positions), neg_target)
            neg_selected = rng.sample(neg_positions, neg_target) if neg_target > 0 else []
        ...
        for y, x in selected_positions:
            ...
            np.save(os.path.join(split_tile_dir, tile_name), tile_features.astype(np.float32))
            np.save(os.path.join(split_label_dir, tile_name), tile_labels.astype(np.uint8))
            tile_records[split_name].append(tile_name)

            valid_pixels = tile_labels != 255
            if np.any(valid_pixels):
                unique, counts = np.unique(tile_labels[valid_pixels], return_counts=True)
                for cls, count in zip(unique, counts):
                    class_pixel_counts[str(int(cls))] = class_pixel_counts.get(str(int(cls)), 0) + int(count)
    ...
    dataset_summary = {
        ...
        'feature_stack_profile': {
            'height': height,
            'width': width,
            'transform': list(profile['transform']) if isinstance(profile['transform'], Affine) else profile['transform'],
        },
    }
